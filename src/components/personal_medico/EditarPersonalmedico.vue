  ...
  export default {
    ...
    methods: {
      obtenerDatosMedico() {
        ...
        axios
          .get(`http://127.0.0.1:8000/api/personal_medico/${medicoId}`)
          .then(response => {
      ...
      updateMedico() {
        const medicoId = this.$route.params.id;
        axios
          .put(`http://127.0.0.1:8000/api/personal_medico/${medicoId}`, this.medico)
          .then(() => {
            this.$router.push({ name: 'personal_medico.index' });
            Swal.fire({
              position: 'top-end',
              icon: 'success',
